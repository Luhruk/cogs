...
class PresidentCog(commands.Cog):
    ...
    async def check_results(self, msg_id, guild_id):
        ...
        if nomination and now >= datetime.fromisoformat(nomination['end_time']):
            msg = await self.bot.get_channel(nomination['channel']).fetch_message(msg_id)  # Updated this line
    ...
    async def process_accepted_nomination(self, msg, user_id):
        ...
        try:
            ...
            if reply.content.lower() == 'i accept':
                await self.bot.get_channel(reply_msg.channel.id).send(f'Congratulations, {user.name}! You are now the president.')
